...
NOTE: this workflow is based on the scripts (NI04 & NI08) provided at https://github.com/czbiohub/scell_lung_adenocarcinoma
...
Subset into cancer epithelial cells only. Then look at deferentially expressed genes in 1. TN vs PER, 2. TN vs PD, and 3. PER vs PD.
...
Load previous epithelial annotated tumor Seurat object
```{r}
rm(list=ls())
...
ggsave(plot = x1, paste(dir,"plot_out/tumor_nontumor_uniquegenes.pdf", sep=""), width = 90, height = 90, units = "mm")
...
compare CNV_scores
```{r}
cnv_scores <- read.csv(file = paste(dir, "data_out/NI03/cnv_scores.csv", sep = ""))
